...
OVERLAY size_t fwrite2(const int2 *ptr, size_t size, XFILE *file)
{
  int4 rev = 0;
  int i;
  ...
  return xfwrite( &rev, size, 1, file ) ;
}
...
OVERLAY int WriteMIDheader( int2 ntrks, int2 division, XFILE *file )
{
  xfwrite( MIDIMAGIC , 10, 1, file ) ;
...
OVERLAY void WriteTrack( int tracknum, XFILE *file, struct Track track[] )
{
  int2 size ;
  size_t quot, rem ;
  ...
  xfwrite( "MTrk", 4, 1, file ) ;
  if( !tracknum ) size += 33 ;

  fwrite2( &size, 4, file ) ;
  if( !tracknum)
    xfwrite( TRACKMAGIC1 "Quick MUS->MID ! by S.Bacquet", 33, 1, file ) ;
  quot = (size_t) (track[tracknum].current / 4096) ;
  rem = (size_t) (track[tracknum].current - quot*4096) ;
  xfwrite( track[tracknum].data, 4096, quot, file ) ;
  xfwrite( ((const unsigned char *) track[tracknum].data)+4096*quot, rem,
                         1, file ) ;
  xfwrite( TRACKMAGIC2, 4, 1, file ) ;
}


OVERLAY void WriteFirstTrack( XFILE *file )
{
  int2 size ;
  ...
  xfwrite( "MTrk", 4, 1, file ) ;
  fwrite2( &size, 4, file ) ;
  xfwrite( TRACKMAGIC3 , 4, 1, file ) ;
  xfwrite( "QMUS2MID (C) S.Bacquet", 22, 1, file ) ;
  xfwrite( TRACKMAGIC4, 6, 1, file ) ;
  xfwrite( TRACKMAGIC5, 7, 1, file ) ;
  xfwrite( TRACKMAGIC6, 4, 1, file ) ;
}
...
OVERLAY int qmus2mid( const void *mus, size_t len, XFILE *file_mid,
      ...
      if( MUS2MIDchannel[MUSchannel] == -1 )
        {
          ...
          track[MIDItrack].data = (char *) malloc( TRACKBUFFERSIZE );
          printf("MALLOC: %p\n", track[MIDItrack].data);
...
{
    if(argc < 3) {
        printf("Usage: %s input.mus output.mid\n", argv[0]);
        return 1;
    }
    ...
    unsigned char *mus_data = malloc(filesize);
    if(!mus_data) {
    ...
    if(fread(mus_data, 1, filesize, fin) != (size_t)filesize) {
    ...
    printf("Found .mus file: %ld bytes\n", filesize);
...
#endif
    // Call converter
    int ret = qmus2mid(mus_data, filesize, fout,
                      0,    // nodisplay = 0 to show messages
                      89,   // default division (ticks per quarter note)
                      64,   // buffer size (in KB)
                      0     // no compression
    );
...
#if USE_MEM
    FILE *memfout = fopen(argv[2], "wb");
    fwrite(mf.buffer, 1, mf.size, memfout);
    fclose(memfout);
    printf("MEMFILE written: %ld bytes\n", mf.size);
#endif
    if(ret) {
